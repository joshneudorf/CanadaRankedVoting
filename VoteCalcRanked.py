...
results = pd.read_csv("2019.txt",sep="\t")
results = results[results.results_type.isin(["validated"])]
print(results)
districts = results.district.unique()
print(len(districts))
results["winner"] = 0

left = ["Liberal","Green Party","NDP-New Democratic Party"]
right = ["People's Party","Conservative","Christian Heritage Party","Bloc Québécois","Parti Rhinocéros Party"]
# right = ["People's Party","Conservative","Christian Heritage Party","Parti Rhinocéros Party"]
results["vote_rank"] = 0
for d in districts:
...
ndp_count = results.loc[results.party == "NDP-New Democratic Party","winner"].sum()
